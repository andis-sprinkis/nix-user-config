...
main() {
  ...
  # alias
  . "${XDG_CONFIG_HOME:-$HOME/.config}/shell/alias.sh"

  # configure bat
  is_exec "bat" && is_exec_bat="1" && { 
    export BAT_THEME="Visual Studio Dark+"
    export BAT_STYLE="plain,numbers,header-filename"
  }

  # configure fzf
  is_exec "fzf" && {
    [ "$is_exec_bat" ] && fzf_bat_opts="--preview 'bat --color=always {}'"
    export FZF_DEFAULT_OPTS="--tabstop=2 --cycle --color=dark --layout=reverse $fzf_bat_opts"
  }
